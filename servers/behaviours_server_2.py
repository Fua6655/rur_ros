...
class NaoBehaviors(NaoqiNode):
    ...
    def __init__( self ):
        ...
        self.behaviorProxy = ALProxy("ALBehaviorManager", self.pip, self.pport)

        # Register ROS services
        self.getInstalledBehaviorsService = rospy.Service(self.pname+"/behaviour/get", GetInstalledBehaviors, self.getInstalledBehaviors)

        #Prepare and start actionlib server
        self.actionlibServer = actionlib.SimpleActionServer(self.pname+"/behaviour/run", RunBehaviorAction, self.runBehavior, False)
