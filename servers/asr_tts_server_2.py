...
class SoundEventWatcher(object):
    ...
    def __init__(self, app):
        ...
        self.pname = rospy.get_param('/pepper_name')
        self.pub_word = rospy.Publisher(self.pname+"/asr/word", String, queue_size=10)
        self.pub_word_recognized = rospy.Publisher(self.pname+"/asr/word_recognized", WordRecognized, queue_size=10)
        self.pub_word_and_grammar = rospy.Publisher(self.pname+"/asr/word_and_grammar", String, queue_size=10)
        self.pub_speech = rospy.Publisher(self.pname+"/asr/speech", String, queue_size=10)
    ...
    def on_word_recognized(self, value):
        ...
        if len(value) > 0:  # found a new word
            ...
            #print value[0]
            self.a,self.b,self.c = value[0].split(" ")
            #print b
            self.goal = WordRecognized()
            self.goal.words = [self.b]
            self.goal.confidence_values = [value[1]]
            self.pub_word_recognized.publish(self.goal)
            rate.sleep()
